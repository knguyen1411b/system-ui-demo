        ...
        <div className="min-h-screen bg-[#0f172a] text-slate-200 p-8 space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                ...
                <select
                    ...
                    className="bg-white/5 border border-white/10 rounded-2xl px-4 py-3 outline-none focus:border-purple-500 text-sm text-gray-300"
                >
                        ...
                        {filteredRooms.map(r => (
                            <tr key={r.id} className="hover:bg-white/5 transition-all">
                                ...
                                <td className="px-6 py-4">
                                    <span
                                        className={`px-2 py-1 rounded-md text-[10px] font-black uppercase border ${r.type === 'VIP' ? 'border-amber-500/50 text-amber-400 bg-amber-500/10' : 'border-blue-500/50 text-blue-400 bg-blue-500/10'}`}
                                    >
                                        {r.type}
                                    </span>
                                </td>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-2">
                                        <div
                                            className={`w-2 h-2 rounded-full ${r.status === 'Đang hoạt động' ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]' : 'bg-gray-500'}`}
                                        ></div>
                                        <span
                                            className={`${r.status === 'Đang hoạt động' ? 'text-emerald-400' : 'text-gray-400'} font-semibold`}
                                        >
                                            {r.status}
                                        </span>
                                    </div>
                                ...
                                <td className="px-6 py-4 text-right space-x-2">
                                    <button
                                        onClick={() => setViewModal({ open: true, data: r })}
            ...
            {formModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
                    <form
                        ...
                        className="bg-[#161b33] border border-white/10 p-6 rounded-3xl w-full max-w-md space-y-4"
                    >
                        <h4 className="text-lg font-bold">
                            {formModal.mode === 'edit' ? 'Cập nhật phòng' : 'Thêm phòng mới'}
                        </h4>

                        <div className="space-y-3">
                            <div>
                                <label className="text-[11px] text-gray-400 font-bold uppercase block mb-1">
                                    Mã phòng
                                </label>
                                <input
                                    type="text"
                    ...
                    <div className="bg-[#161b33] border border-white/10 p-6 rounded-3xl w-full max-w-sm space-y-4">
                        ...
                        <div className="space-y-2 text-sm border-t border-white/5 pt-3">
                            <p>
                                ...
                                <span className="font-mono">{viewModal.data.code}</span>
                            </p>
                            <p>
                                <span className="text-gray-400">Loại:</span> {viewModal.data.type}
                            </p>
                            <p>
                                <span className="text-gray-400">Trạng thái:</span> {viewModal.data.status}
                            </p>
                        </div>
