...
export default function QuanLyTaiKhoan() {
    ...
    const filteredAccounts = accounts.filter(acc => {
        const matchSearch =
            acc.name.toLowerCase().includes(searchQuery.toLowerCase()) || acc.phone.includes(searchQuery)
                            ...
                            filteredAccounts.map(a => (
                                <tr key={a.id} className="border-b border-white/5 hover:bg-white/5 transition-all">
                                    ...
                                    <td className="px-6 py-4">
                                        <span
                                            className={`px-2 py-1 rounded-md text-[9px] font-black uppercase border ${getBadgeStyle(a.role)}`}
                                        >
                                            {a.role}
                                        </span>
                                    </td>
            ...
            {isFormModalOpen && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <form
                        ...
                        className="glass w-full max-w-md rounded-[2.5rem] p-8 relative z-10 bg-[#161b33]"
                    >
                        <h4 className="text-xl font-bold mb-6">
                            {formData.id ? 'Cập nhật thành viên' : 'Thêm thành viên'}
                        </h4>
                        ...
                        <div className="flex gap-3 mt-8">
                            <button
                                type="button"
                                onClick={() => setIsFormModalOpen(false)}
                                className="flex-1 py-3 text-sm font-bold opacity-50 hover:opacity-100 text-white"
                            >
                                Hủy
                            </button>
                            <button
                                type="submit"
                                className="flex-1 py-3 bg-purple-600 rounded-xl text-sm font-bold hover:bg-purple-500 shadow-lg shadow-purple-600/20 text-white"
                            >
                                Xác nhận
                            </button>
                        </div>
            ...
            {isViewModalOpen && selectedAccount && (
                <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
                    ...
                    <div className="glass w-full max-w-sm rounded-[2.5rem] p-8 relative z-10 bg-[#161b33] text-center">
                        ...
                        <div className="space-y-3 text-left bg-white/5 p-4 rounded-2xl border border-white/10">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-bold text-emerald-400">
                                    {selectedAccount.role === 'Khách hàng'
                                        ? `${selectedAccount.points.toLocaleString()} pts`
                                        : 'Không áp dụng'}
                                </span>
                            </div>
                        </div>
                        <button
                            onClick={() => setIsViewModalOpen(false)}
                            className="mt-8 w-full py-3 bg-white/10 rounded-xl font-bold hover:bg-white/20 text-white"
                        >
                            Đóng
                        </button>
                    </div>
