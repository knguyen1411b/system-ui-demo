...
export default function LichSuThuePhong() {
    ...
    const [history] = useState([
        ...
        },
        {
            ...
            duration: 2.5,
            roomPrice: '1.000.000',
            serviceTotal: '500.000',
            total: '1.500.000',
            roomImg: 'https://images.unsplash.com/photo-1516280440614-37939bbacd81?w=400',
            services: [
                {
                    name: 'Trái cây dĩa (Nhỏ)',
                    qty: 1,
                    price: '120.000',
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-2">
                {history.slice(0, visibleItems).map(item => (
                    <div
                        key={item.id}
                        className="bg-white/[0.01] border border-white/5 rounded-xl overflow-hidden hover:border-[#a78bfa] transition-all group flex items-center gap-3 p-3"
                    >
                        <img
                            src={item.roomImg}
            ...
            {visibleItems < history.length && (
                <div className="mt-16 flex justify-center">
                    <button
                        ...
                        className="group flex flex-col items-center gap-2 text-[#a78bfa] hover:text-white transition-all"
                    >
                ...
                <div className="fixed inset-0 z-[999] flex items-center justify-center p-2">
                    ...
                    <div className="bg-slate-900 border border-white/10 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl relative z-10">
                        ...
                        <div className="p-4 overflow-y-auto [scrollbar-width:none] [-webkit-scrollbar:none] max-h-[60vh]">
                            {/* Chi phí phòng */}
                            <div className="mb-6">
                                <h5 className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-3 px-1">
                                    Chi phí thuê phòng
                                </h5>
                                <div className="flex items-center gap-3 p-3 rounded-xl bg-white/[0.03] border border-white/5 group">
                                    <img
                                        src={selectedBooking.roomImg}
                                    ...
                                    <div className="flex-grow min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 font-semibold">
                                            {selectedBooking.in} - {selectedBooking.out} |{' '}
                                            {selectedBooking.duration.toFixed(1)} Giờ
                                        </p>
                                    </div>
                                ...
                                <div className="space-y-1.5">
                                    {selectedBooking.services.map((s, index) => {
                                        ...
                                        return (
                                            <div
                                                key={index}
                                                className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.01] border border-white/5 hover:bg-white/[0.03] transition-colors"
                                            >
                                                <img
                                                    src={s.img}
                                                    className="w-8 h-8 rounded object-cover bg-slate-800 border border-white/10"
                                                    alt={s.name}
                                                />
                                                <div className="flex-grow min-w-0">
                        ...
                        <div className="p-4 bg-slate-800/50 border-t border-white/5">
                            <div className="flex justify-between items-center mb-4 border border-emerald-500/20 bg-emerald-500/5 p-3 rounded-xl">
                                <div className="text-left">
                                    <p className="text-[10px] text-emerald-300 font-bold uppercase tracking-widest mb-1">
                                        Tổng cộng
                                    </p>
                            ...
                            <button
                                onClick={() => setShowDetail(false)}
                                className="w-full py-3 bg-white text-slate-900 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-purple-600 hover:text-white transition-all shadow"
                            >
                                Đóng hóa đơn
                            </button>
                        </div>
