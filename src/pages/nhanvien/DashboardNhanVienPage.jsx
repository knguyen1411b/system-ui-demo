...
export default function DashboardNhanVienPage() {
    ...
    <div className="w-[260px] h-full bg-[#0f172a]/60 text-slate-200 border-r border-white/5 flex flex-col backdrop-blur-xl">
      ...
      <div className="m-5 p-4 bg-white/[0.02] border border-white/5 rounded-[2rem] flex items-center gap-3 backdrop-blur-[5px]">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-[#a78bfa] font-black uppercase tracking-widest mt-0.5">Nhân viên</span>
        </div>
      </div>
      ...
      <nav className="flex-1 px-4 space-y-3 overflow-y-auto">
        ...
        <div className="space-y-1.5">
          <div
            ...
            className={`px-4 py-3 flex justify-between items-center rounded-xl cursor-pointer transition-all duration-300 ${
              isRoomGroupOpen ? 'text-white bg-white/5' : 'text-gray-400 hover:text-[#a78bfa] hover:bg-white/5'
            }`}
          >
            <div className="flex items-center gap-3.5 text-sm font-bold tracking-wide uppercase">
              <LayoutDashboard className={`w-4 h-4 transition-colors ${isRoomGroupOpen ? 'text-[#8b5cf6]' : 'text-gray-400'}`} />
              <span>Quản lý phòng</span>
            </div>
            <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isRoomGroupOpen ? 'rotate-180 text-[#8b5cf6]' : 'text-gray-500'}`} />
          </div>
          ...
          <div className={`pl-12 space-y-1 transition-all duration-300 overflow-hidden ${isRoomGroupOpen ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
            <NavLink
              ...
              style={({ isActive }) => isActive ? { textShadow: '0 0 10px rgba(167, 139, 250, 0.4)' } : {}}
            >
              Thuê phòng
            </NavLink>
        ...
        <div className="space-y-1.5">
          <div
            ...
            className={`px-4 py-3 flex justify-between items-center rounded-xl cursor-pointer transition-all duration-300 ${
              isRequestGroupOpen ? 'text-white bg-white/5' : 'text-gray-400 hover:text-[#a78bfa] hover:bg-white/5'
            }`}
          >
            <div className="flex items-center gap-3.5 text-sm font-bold tracking-wide uppercase">
              <BellRing className={`w-4 h-4 transition-colors ${isRequestGroupOpen ? 'text-[#8b5cf6]' : 'text-gray-400'}`} />
              <span>Tiếp nhận yêu cầu</span>
            </div>
            <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${isRequestGroupOpen ? 'rotate-180 text-[#8b5cf6]' : 'text-gray-500'}`} />
          </div>
  ...
  return (
    <div className="min-h-screen w-full bg-[#0f172a] bg-gradient-to-br from-[#1e1b4b] to-[#0f172a] text-slate-200 flex overflow-hidden antialiased font-['Plus_Jakarta_Sans',sans-serif] relative">
      ...
      <aside className={`fixed inset-y-0 left-0 h-full z-50 lg:hidden transform transition-transform duration-300 ease-in-out ${
      ...
      <div className="flex-1 flex flex-col min-w-0 lg:pl-[260px] print:pl-0">
        ...
        <header className="lg:hidden flex items-center justify-between px-6 py-4 bg-[#0f172a]/80 border-b border-white/5 backdrop-blur-xl sticky top-0 z-30">
          ...
          <span className="text-sm font-black tracking-widest text-[#a78bfa]" style={{ textShadow: '0 0 10px rgba(167, 139, 250, 0.4)' }}>
            MUSICBOX CONTROL
          </span>
