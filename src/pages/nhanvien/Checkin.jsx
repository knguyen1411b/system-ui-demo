    ...
    return (
        <div className="min-h-screen bg-[#0f172a] p-4 sm:p-8 lg:p-10 text-slate-200 font-['Plus_Jakarta_Sans',sans-serif] relative overflow-hidden">
            ...
            <div className="main-content">
                ...
                <div
                    id="checkInBookingView"
                    className="checkin-view bg-white/5 backdrop-blur-md border border-white/10 p-6 sm:p-8 rounded-3xl shadow-2xl max-w-2xl mx-auto animate-fadeIn"
                >
                    ...
                    {checkinAlert.show && (
                        <div
                            className={`alert-box flex items-center gap-3 p-4 rounded-xl mb-6 text-sm border animate-pulse ${checkinAlert.type === 'warning' ? 'bg-amber-500/10 border-amber-500/30 text-amber-300' : 'bg-rose-500/10 border-rose-500/30 text-rose-300'}`}
                        >
                            <i
                                className={`fas ${checkinAlert.type === 'warning' ? 'fa-exclamation-triangle' : 'fa-times-circle'} text-base`}
                            ></i>
                    ...
                    {checkinStep === 'scanner' && (
                        <div
                            id="checkinFormBody"
                            className="space-y-6"
                            style={{ opacity: isCheckinFormDisabled ? 0.3 : 1 }}
                        >
                            <div className="checkin-scanner-box relative bg-black/30 border border-white/10 rounded-2xl p-6 text-center flex flex-col items-center justify-center overflow-hidden group">
                                ...
                                <p className="text-sm font-semibold tracking-wide">
                                    HỆ THỐNG ĐANG QUÉT MÃ QR KHÁCH HÀNG...
                                </p>
                                <p className="text-xs text-slate-400 mt-1">
                                    (Đặt mã đặt chỗ của khách hàng trước mắt camera)
                                </p>
                            </div>

                            <div className="checkin-input-wrapper flex flex-col space-y-2">
                                <label className="text-xs text-slate-300 font-medium">
                                    Hoặc nhập mã đặt phòng thủ công:
                                </label>
                                <input
                                    type="text"
                                    id="bookingCodeInput"
                    ...
                    {checkinStep === 'success' && (
                        <div
                            id="checkinSuccessBody"
                            className="success-result-box flex flex-col space-y-6 animate-fadeIn"
                        >
                            <div
                                id="successAlert"
                                className={`alert-box border flex items-center gap-3 p-4 rounded-xl text-sm transition-all ${isRented ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-indigo-500/10 border-indigo-500/30 text-indigo-300'}`}
                            >
                                <i
                                    className={`fas ${isRented ? 'fa-check-circle text-emerald-400' : 'fa-info-circle text-indigo-400'} text-base`}
                                ></i>
                                <span>
                                    {isRented
                                        ? 'Thuê phòng thành công! Phòng đã được kích hoạt sử dụng.'
                                        : 'Xác thực thành công! Vui lòng kiểm tra thông tin và ấn nút Thuê Phòng.'}
                                </span>
                            </div>
                            ...
                            <div className="info-card bg-black/20 border border-white/5 rounded-2xl p-4 space-y-3 text-sm">
                                <div className="info-row flex justify-between border-b border-white/5 pb-2.5">
                                    ...
                                    <span id="resBookingCode" className="font-mono font-bold text-[#c4b5fd]">
                                        {successData.code}
                                    </span>
                                </div>
                                <div className="info-row flex justify-between border-b border-white/5 pb-2.5">
                                    <span className="text-slate-400">Tên khách hàng</span>
                                    <span id="resCustomerName" className="font-medium text-white">
                                        {successData.name}
                                    </span>
                                </div>
                                ...
                                <div className="info-row flex justify-between border-b border-white/5 pb-2.5">
                                    ...
                                    <span className="font-medium text-[#c4b5fd] font-semibold">
                                        {successData.usageDate}
                                    </span>
                                </div>

                                <div className="info-row flex justify-between border-b border-white/5 pb-2.5">
                                    <span className="text-slate-400">Giờ vào dự kiến</span>
                                    <span className="font-medium text-emerald-400 font-semibold">
                                        {successData.checkInTime}
                                    </span>
                                </div>
                                <div className="info-row flex justify-between border-b border-white/5 pb-2.5">
                                    <span className="text-slate-400">Giờ ra dự kiến</span>
                                    <span className="font-medium text-rose-400 font-semibold">
                                ...
                                <div className="info-row flex justify-between pt-0.5">
                                    ...
                                    <span>
                                        <span
                                            className={`text-xs px-2.5 py-1 rounded-full border font-bold transition-all ${isRented ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'bg-amber-500/20 text-amber-300 border-amber-500/30'}`}
                                        >
                            ...
                            {isRented ? (
                                <div className="ticket-print-zone bg-white/5 p-6 rounded-2xl border border-dashed border-white/20 flex flex-col items-center text-center space-y-3 animate-fadeIn">
                                    <span
                                        className="qr-caption block text-sm font-bold tracking-wider text-slate-200"
                                        id="printTicketCaption"
                                    >
                                    ...
                                    <p className="text-[11px] text-slate-400 max-w-sm">
                                        Mã QR dùng để kích hoạt hệ thống âm thanh ánh sáng tự động tại cửa phòng{' '}
                                ...
                                <div className="py-5 bg-white/[0.02] border border-dashed border-white/5 rounded-2xl text-center text-xs text-slate-500 italic">
                                    Vui lòng kiểm tra lại thời gian trên và ấn nút "Thuê phòng ngay" bên dưới để kích
                                    hoạt mã QR vào phòng.
                                </div>
                            )}
