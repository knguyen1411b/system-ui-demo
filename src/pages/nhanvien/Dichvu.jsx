        ...
        setRoomsData(prevData =>
            prevData.map(room => {
                if (room.roomId === roomId) {
                    return {
                        ...room,
                        items: room.items.map(item => (item.id === itemId ? { ...item, status: newStatus } : item))
                    }
                }
                return room
            })
                                ...
                                <tbody className="divide-y divide-white/[0.02]">
                                    {room.items.map(item => (
                                        <tr key={item.id} className="hover:bg-white/[0.01] transition-colors">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <span className="text-base font-bold text-[#c4b5fd] font-mono">
                                                    {item.quantity}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <select
                                                    value={item.status}
                                                    onChange={e =>
                                                        handleStatusChange(room.roomId, item.id, e.target.value)
                                                    }
                                                ...
                                                >
                                                    <option value="Chờ xử lý" className="bg-[#1e1b4b]">
                                                        Chờ xử lý
                                                    </option>
                                                    <option value="Đã giao" className="bg-[#1e1b4b]">
                                                        Đã giao
                                                    </option>
                                                    <option value="Hủy" className="bg-[#1e1b4b]">
                                                        Hủy
                                                    </option>
                                                </select>
                                            </td>
