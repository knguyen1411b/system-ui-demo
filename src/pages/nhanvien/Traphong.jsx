...
const parseTimeToDate = (timeString, referenceDate = new Date()) => {
  const [hours = 0, minutes = 0] = String(timeString || '').trim().split(':').map(Number);
...
const INITIAL_ROOMS = [
  {
    ...
    services: [
      { name: 'Bia Tiger', qty: 6, price: '25.000', img: 'https://images.unsplash.com/photo-1510812431401-41d2bd2722f3?w=100' },
      { name: 'Trái cây đĩa (Lớn)', qty: 1, price: '150.000', img: 'https://images.unsplash.com/photo-1519996529931-28324d5a630e?w=100' },
      { name: 'Snack Oishi', qty: 3, price: '15.000', img: 'https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=100' }
    ]
  },
  {
    id: '105',
    ...
    services: [
      ...
      { name: 'Hạt hướng dương', qty: 1, price: '15.000', img: 'https://images.unsplash.com/photo-1519996529931-28324d5a630e?w=100' }
    ]
  },
  {
    id: '201',
    ...
    roomPricePerHour: 80000,
    services: [
      { name: 'Coca Cola', qty: 4, price: '15.000', img: 'https://images.unsplash.com/photo-1548839140-29a749e1cf4d?w=100' }
    ]
  }
];
...
export default function TraPhongPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] p-4 sm:p-8 lg:p-10 text-slate-200 font-['Plus_Jakarta_Sans',sans-serif] relative overflow-hidden">
      ...
      {viewingServicesRoom && (
        <ServiceDetailsModal
          room={viewingServicesRoom}
          onClose={() => setViewingServicesRoom(null)}
        />
      )}
      ...
      <div>
        ...
        {room.ThoiGianDuKienRa && (
          <div className="mb-4 bg-amber-500/10 border border-amber-500/20 text-amber-400 p-2.5 rounded-xl flex items-center justify-between text-xs font-semibold">
            <span><i className="fas fa-hourglass-start mr-1.5 text-amber-400"></i>Thời gian còn lại:</span>
            <span className="font-mono tracking-wide bg-amber-500/20 px-2 py-0.5 rounded text-amber-300 font-bold">{timeLeft}</span>
          </div>
        )}
        ...
        <div className="space-y-2.5 text-sm mb-4">
          <div className="text-slate-300 flex items-center gap-2.5">
            ...
            <span className="text-slate-400">Khách: <strong className="text-slate-200 font-semibold">{room.customerName}</strong></span>
          </div>
          <div className="text-slate-300 flex items-center gap-2.5">
            <i className="fas fa-phone w-4 text-slate-400 text-center text-xs"></i>
            <span className="text-slate-400">SĐT: <strong className="text-slate-200 font-semibold">{room.phone || <span className="text-amber-500/80 italic font-normal">Trống</span>}</strong></span>
          </div>
          <div className="text-slate-300 flex items-center gap-2.5">
            <i className="fas fa-clock w-4 text-slate-400 text-center text-xs"></i>
            <span className="text-slate-400">Giờ vào: <strong className="text-emerald-400 font-semibold">{room.timeIn}</strong></span>
          </div>
          <div className="text-slate-300 flex items-center gap-2.5">
            <i className="fas fa-sign-out-alt w-4 text-slate-400 text-center text-xs"></i>
            <span className="text-slate-400">Giờ ra dự kiến: <strong className="text-slate-300 font-semibold">{room.ThoiGianDuKienRa || 'Hát tự do'}</strong></span>
          </div>
        </div>
...
function ServiceDetailsModal({ room, onClose }) {
  const totalServiceCost = room.services.reduce((acc, curr) => acc + (parseFloat(curr.price.replace(/\./g, '')) * curr.qty), 0);
      ...
      <div className="bg-[#1e1b4b] border border-white/10 text-white w-full max-w-[500px] p-6 rounded-2xl shadow-2xl transition-all">
        ...
        <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
          {room.services.map((service, index) => (
            <div key={index} className="flex items-center gap-3 bg-white/5 p-2.5 rounded-xl border border-white/5">
              <img src={service.img} alt={service.name} className="w-12 h-12 rounded-lg object-cover bg-slate-800" />
              ...
              <div className="text-right">
                ...
                <div className="text-sm font-bold text-white mt-0.5">
                  {(parseFloat(service.price.replace(/\./g, '')) * service.qty).toLocaleString('vi-VN')}đ
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="border-t border-white/10 pt-4 mt-4 flex justify-between items-center">
          <div>
            ...
            <span className="text-lg font-black text-orange-400">{totalServiceCost.toLocaleString('vi-VN')}đ</span>
          </div>
          <button onClick={onClose} className="bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-xl font-bold text-sm transition">
            Đóng
          </button>
        </div>
      </div>
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[9999] flex justify-center items-center p-4">
      <div className="bg-[#1e1b4b] border border-violet-500/30 text-white w-full max-w-[450px] p-6 rounded-2xl shadow-2xl text-center">
        ...
        <p className="text-slate-300 text-sm mb-4">
          Hệ thống sẽ thực hiện chốt giờ dùng và tính tiền hóa đơn ngay lập tức cho <span className="text-[#c4b5fd] font-bold">Phòng {room.id}</span>.
        </p>

        <div className="bg-black/30 border border-white/5 rounded-xl p-4 text-left space-y-2 mb-4 text-sm">
          <div className="flex justify-between"><span className="text-slate-400">Giờ khách vào:</span><b className="text-white">{room.timeIn}</b></div>
          <div className="flex justify-between"><span className="text-slate-400">Giờ ra hiện tại (chốt):</span><b className="text-emerald-400 font-black">{currentTimeStr}</b></div>
          <div className="flex justify-between"><span className="text-slate-400">Tổng thời gian sử dụng:</span><b className="text-[#c4b5fd] font-bold">{durationText}</b></div>
        </div>
        ...
        <div className="text-left mb-6">
          ...
          <select
            className="w-full bg-black/40 border border-violet-500/20 rounded-xl px-4 py-3 text-sm text-slate-200 font-medium focus:outline-none focus:border-violet-500 transition cursor-pointer appearance-none bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23a78bfa%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E')] bg-[length:12px_12px] bg-[right_16px_center] bg-no-repeat pr-10"
          >
            <option value="NGAY_THUONG" className="bg-[#1e1b4b] text-white">Ngày thường</option>
            <option value="CUOI_TUAN" className="bg-[#1e1b4b] text-white">Cuối tuần</option>
            <option value="NGAY_LE" className="bg-[#1e1b4b] text-white">Ngày lễ</option>
          </select>
        </div>
    ...
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-[9999] flex justify-center items-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-[#1e1b4b]/90 border border-white/10 text-white w-full max-w-[550px] p-5 sm:p-8 rounded-2xl shadow-2xl backdrop-blur-xl max-h-[92vh] overflow-y-auto transform transition-all print:absolute print:inset-0 print:bg-white print:text-black print:p-0 print:max-h-none">
        ...
        <div className="text-center mb-5 border-b-2 border-white/20 pb-4 print:border-slate-800">
          ...
          {/* HIỂN THỊ MÃ HÓA ĐƠN */}
          <p className="text-xs font-bold text-amber-400 mt-1 tracking-widest print:text-slate-800">MÃ HĐ: {invoiceId}</p>
        ...
        <div className="mb-5 text-sm space-y-2 border-b border-dashed border-white/10 pb-4 print:border-slate-300 print:text-black">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <p>
              ...
              <b className="text-white print:text-black">{phoneNumber || <span className="text-amber-400 italic font-normal">(Chưa thiết lập)</span>}</b>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 pt-1 border-t border-white/5 print:border-slate-100">
            ...
            <p><span className="text-slate-400 print:text-slate-500">Ngày thanh toán:</span> <b className="text-white print:text-black">{new Date().toLocaleDateString('vi-VN')}</b></p>
          </div>
          ...
          <div className="grid grid-cols-3 gap-2 pt-2 mt-1 text-xs bg-black/20 p-2.5 rounded-xl border border-white/5 print:bg-slate-50 print:border-slate-200 print:text-black">
            ...
            <div className="text-center border-r border-white/10 print:border-slate-200">
              ...
              <b className="text-rose-400 font-bold text-sm print:text-black">{room.timeOut || new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit', hour12: false })}</b>
            </div>
        ...
        <div className="w-full overflow-x-auto mb-5">
          <table className="w-full text-left border-collapse text-sm min-w-[420px]">
            ...
            <tbody className="divide-y divide-white/5 print:divide-slate-100">
              {room.items && room.items.map((item, idx) => (
        ...
        <div className="mt-6 p-4 bg-black/20 border border-dashed border-white/10 rounded-xl flex flex-col items-center justify-center gap-2 print:border-none print:bg-white print:mt-4">
          ...
          {qrUrl ? (
            <img src={qrUrl} alt="QR Code" className="w-32 h-32 border-4 border-white/10 bg-white p-1 shadow-md rounded-lg print:border-slate-200" />
          ) : (
